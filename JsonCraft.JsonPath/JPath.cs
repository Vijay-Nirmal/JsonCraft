...
namespace JsonCraft.JsonPath
{
    public class JPath
    {
        ...
        private static readonly JsonElement TrueJsonElement = JsonSerializer.SerializeToElement(true);
        ...
        private bool TryParseValue(out JsonElement value)
        {
            char currentChar = _expression[_currentIndex];
            if (currentChar == '\'')
            {
                value = JsonSerializer.SerializeToElement(ReadQuotedString());
                return true;
            }
            ...
            else if (currentChar == 'n')
            {
                if (Match("null"))
                {
                    value = NullJsonElement;
                    return true;
                }
            }
            else if (currentChar == '/')
            {
                value = JsonSerializer.SerializeToElement(ReadRegexString());
                return true;
            }
        ...
        internal IEnumerable<JsonElement> Evaluate(JsonElement root, JsonElement t, JsonSelectSettings? settings)
        {
